module Visualization
    ...
    function DrawHeatMapNS!(xs, ys, dens, col, colorrange)
        ...
        elseif col == "nemD"
            ...
            hm = GLMakie.heatmap!(xs, ys, dens, colorrange=colorrange, interpolate = false) # (-5e-4, 5e-4)
        end
        return hm
    end
    ...
    function StaticArrows(t, parameters, densityArray, velocityArray, directorArray, nematicArray, phibArray, phiuArray, sigmaVEArray,
        ...
        colorrange = (0.99, 1.001), recording = false, imagePath = "", res = 2000, ar = 1, cScaleFac = 1,
        IBBool = false, tracking = false, p1Bool = false, IBlinewidth = 6.0, IBmarkersize = 15,
        iColor = :black, eColor1 = :red, eColor2 = :red, cColor = :black, oColor = :orange, tColor = :white, p1Color = :orange,
        ...
        #Observable(arrowFac * [Point2(divS.XValues[i,j], divS.YValues[i,j]) for i = 1:grid.Nx, j = 1:grid.Ny])
        cols = [(:black, log(VectorNorm2D([duVecs[][x,y][1], duVecs[][x,y][2]])) ) for x in 1:size(duVecs[])[1], y in 1:size(duVecs[])[2]]
        if arrows != "nem"
            #arrows!(apts, duVecs, linewidth = arrowWidth, arrowsize = arrowHead, color = (:black, arrowOp), aspect = 1)
            arrows!(apts, duVecs, linewidth = arrowWidth, arrowsize = arrowHead, arrowcolor = cols, aspect = 1)
        else 
            arrows!(apts[1], apts[2], duVecs[1], duVecs[2], linewidth = arrowWidth, arrowsize = arrowHead, color = (:black, arrowOp), aspect = 1)
        end 
        if IBBool
    ...
    function StaticStreamlines(t, parameters, # add defect tracking
        ...
        ObservablePositionArray, rExtArray, kArray;
        col = "dens", stream = "vel", colorrange = (0.99, 1.001), recording = false, imagePath = "", res = 2000, cScaleFac = 1,
        streamArrowSize = 0.1, streamLineWidth = 4.0, streamDensity = 3.0, streamStepSize = 0.01,
        IBBool = false, tracking = false, p1Bool = false, IBlinewidth = 6.0, IBmarkersize = 15,
        iColor = :black, eColor1 = :red, eColor2 = :red, cColor = :black, oColor = :orange, tColor = :white, p1Color = :orange,
        fontsize = 35, ticks = [1], tickLabels = [""], fac = 10^6, xlabel = "", ylabel = "", clabel = "",
        ...
        if stream != "vel"
            if stream == "BE"
                velocityArray = [BerisEdwards.GetUnitDirectorFromTensor2DSoA(grid, nematicArray[t]) for t = 1:length(nematicArray)]
            elseif stream == "divS"
                velocityArray = [Analysis.DivS(grid, sigmaVEArray[t], parameters["bcVE_X"], parameters["bcVE_Y"]) for t = 1:length(sigmaVEArray)]
            else
                println("stream not recognized")
            end
